...
zeroyt7.on("group-participants-update", async (anu) => {
                ...
                teks = `*Selamat Datang Kak @${mem.split('@')[0]}*\n*Di Group / Gc -* *${groupName}*\n*Silahkan Introoo*\n\n*Nama :*\n*Umur :*\n*Askot :*`
                buff = await getBuffer(pp_user)
                zeroyt7.sendMessage(mdata.id, { contentText: `${teks}`, footerText: `- RyGiiXyzzz`, buttons: [{buttonId: `#rulesgroup`,buttonText:{displayText: 'Rules Group'},type:1},{buttonId: `#menu`,buttonText:{displayText: 'Menu'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
		        }
                ...
                buff = await getBuffer(pp_user)
                zeroyt7.sendMessage(mdata.id, { contentText: `${out}`, footerText: `- RyGiiXyzzz`, buttons: [{buttonId: `!bay`,buttonText:{displayText: 'Byeee Ajg'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
                }
                ...
                pa = `*[ PROMOTEDETECTED ]*\n\n*Nomor : ${num.split("@")[0]}*\n*Tag : @${num.split("@")[0]}*\n*Group : ${mdata.subject}*`
                buff = await getBuffer(pp_user)
                zeroyt7.sendMessage(mdata.id, { contentText: `${pa}`, footerText: `*Selamat Kamu Menjadi Admin*`, buttons: [{buttonId: `!menu`,buttonText:{displayText: 'Menu'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
                }
                ...
                nani = `*[ DEMOTEDETECTED ]*\n\n*Nomor : ${num.split("@")[0]}*\n*Tag : @${num.split("@")[0]}*\n*Group : ${mdata.subject}*`
                buff = await getBuffer(pp_user)
                zeroyt7.sendMessage(mdata.id, { contentText: `${nani}`, footerText: `*Kasian Bukan Admin Lagi Xixi*`, buttons: [{buttonId: `!menu`,buttonText:{displayText: 'Menu'},type:1}],headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: buff, contextInfo: {"mentionedJid": [num]}}}, 'buttonsMessage')
                }
    } catch (e) {
