...
let botname = 'RyGiiXyzzz' //NAMA BOT
...
let ownername = 'RyGii' //NAMA OWNER
let owner = '6285755825621' // NOMOR OWNER
let bold = '*' 
let petik = '```'
let faketeks = `*- RyGiiXyzzz*` 
...
const kuz = "6285755825621@s.whatsapp.net"
...
const daftar1 = `Hai kak  ${pushname} ${ucapanWaktu}\nSebelum Menggunakan Fitur Bot Daftar Terlebih Dahulu Ya`
const daftar2 = 'Ketik Tombol Di Bawah Untuk Daftar Kak\nBisa Juga Dengan Ketik !Daftar'
const daftar3 = [{buttonId: `!verify`,buttonText: {displayText: `Daftar`,},type: 1,},]
const kon1 = `*Hai Kak ${pushname}* *Nama Owner Ku RyGii*`
const kon2 = `${faketeks}`
...
const fakeyt = (teks) => { 
zeroyt7.sendMessage(from, teks, text,{contextInfo :{text: 'hi', "forwardingScore": 1000000000, isForwarded: false, sendEphemeral: false, "externalAdReply": { "title": `Oyy! ${pushname} Anjg` , "body": `Verified RyGiiXyzzz`, "mediaType": "2", "thumbnailUrl": "https://c.top4top.io/p_2087f30hj1.jpeg", "mediaUrl": "https://youtu.be/hiCXp2lbAHA", "thumbnail": fs.readFileSync('thumbnail.jpg'), "sourceUrl": "", },mentionedJid:[sender]}, quoted : fstatus})};
...
const fstatus = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "RyGiiXyzzz", 'jpegThumbnail': fs.readFileSync('thumbnail.jpg')}}}
...
zeroyt7.sendMessage(from, { contentText: `${ok}`, footerText: `*- RyGiiXyzzz*`, buttons : [{ buttonId: `.botstat`, buttonText: { displayText: 'BOT STATISTIC' }, type: 1 },{ buttonId: `.sewabot`, buttonText: { displayText: 'SEWA BOT' }, type: 1 }], 
...
break
case 'pdf':
otod = "6285755825621@s.whatsapp.net"
dtod = `${sender}`
uwu = '```'
tescok = `*Fake Doc RyGiiXyzzz*`
...
reply(`*Sukses Fitur Wellcome Aktif* *${groupMetadata.subject}*`)
} else if ((args[0]) === '0') {
if (!isWelkom) return reply('*Fitur Welcome Sdah Off Sebelumnya*')
_welkom.splice(from, 1)
fs.writeFileSync('./F2/welcome.json', JSON.stringify(_welkom))
reply(`*Sukses Fitur Wellcome DiNonaktifkan* *${groupMetadata.subject}*`)
} else {
...
costum(buffer, sticker, Verived, `RyGiiXyzzz Verified`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
case 'sound1':
      ...
      zeroyt7.sendMessage(from, omkeh, MessageType.audio, { quoted: fvn, mimetype: 'audio/mp4', ptt: true })
      break
      case 'sound71':
      case 'sound71':
      case 'sound72':
...
case 'nulis':
case 'tulis':
if (args.length < 1) return reply('Mau Di Tulis Apa Njeng?')
teks = args.join(' ')
reply(mess.wait)
...
zeroyt7.sendMessage(from, { contentText: `${menu}`, footerText: `RyGiiXyzzz
@${Verived.split('@')[0]}
@${zer.split("@")[0]}
@${kuz.split("@")[0]}`, 
...
listmsg(from, `Mau Buy Apaan Kak <3`, `  `, list)
break
case 'belipremium':
case 'buypremium':
case 'sewabot':
...
┏━━⬣ MINAT? PM
┃⬡ WA : Wa.me/6285755825621
┃⬡ FB : RyGiiXyzzz
┗━━⬣`
but = but = [{ buttonId: `${prefix}pulsa`, buttonText: { displayText: 'Pulsa' }, type: 1 },{ buttonId: `${prefix}dana`, buttonText: { displayText: 'Dana' }, type: 1 },{ buttonId: `${prefix}allpay`, buttonText: { displayText: 'All Pay' }, type: 1 }]
ok = `RyGiiXyzzz`
sendButLoc(from, lolot, ok, img, but)
break
case 'muridbot':
katalog(`Open Murid Bot RyGiiXyzzz
...
Pay : Pulsa - Dana - ALL Pay!

Minat?
Wa.me/6285755825621 [ RyGii ]`)
break
case 'script':
case 'sc':
case 'sourcecode':
...
case 'pulsa':
case 'qrpulsa':
teks = `*Donasi Buat Beli Paket Quota :)*`
gipay = fs.readFileSync('./F4/pulsa.jpg')
trans = `- RyGiiXyzzz <3`
...
break
  ...
  break
  case 'bay':
  reply(`*Sayonara Buat Yang Pergi Gua Tandain Lu. Awas Masuk Lagi*`)
  break
  case 'selamatdatang':
...
case 'groupbot':
case 'botgroup':
reply(`*Masuk Sini Njeng Xixixi*
https://chat.whatsapp.com/KW8k6iBcl56EGRsO2PPfFP`)
break
case "runtime":
case "test":
run = process.uptime();
